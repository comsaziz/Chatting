...
function JohnPhone({ messages, onSendMessage }) {
  ...
  const SendFun = () => {
    if (currentMessage.trim()) {
      ...
      onSendMessage("John", currentMessage, timecal);
      setCurrentMessage("");
    }
  };
          ...
          >
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`chat ${
                  msg.sender === "John" ? "chat-end" : "chat-start"
                }`}
              >
                ...
                <div
                  className={`chat-bubble ${
                    msg.sender === "John" ? "bg-blue-500" : "bg-gray-400"
                  }`}
                >
                  {msg.message}
                </div>
